...
obs_control_panel.html: O painel administrativo onde você controla tudo. Pode ser aberto no navegador ou como uma Doca Customizada no OBS.

obs_lower_thirds_source.html: O arquivo de design dos GCs. Adicione como "Navegador" no OBS.

obs_bible_source.html: O arquivo de design da Bíblia. Adicione como "Navegador" no OBS.

🛠️ Instalação e Configuração

Passo 1: Configurar o OBS Studio

Baixe os arquivos para uma pasta no seu computador.

No OBS, crie uma Cena para seus GCs (ou use a atual).

Adicione uma nova fonte do tipo Navegador (Browser Source):

Nome: GC Source

Arquivo Local: Marque a caixa e selecione o arquivo obs_lower_thirds_source.html.

Largura/Altura: 1920 x 1080 (ou a resolução da sua stream).

CSS Personalizado: Limpe tudo (deixe vazio).

Adicione outra fonte do tipo Navegador:

Nome: Bible Source

Arquivo Local: Marque a caixa e selecione o arquivo obs_bible_source.html.
...
No OBS, vá em Ver (View) -> Docas (Docks) -> Docas de Navegador Personalizadas (Custom Browser Docks).

Nome da Doca: Controlador Ultimate

URL: Cole o caminho do arquivo no seu PC.

Dica: Abra o arquivo obs_control_panel.html no Chrome, copie o endereço da barra (ex: file:///C:/Users/SeuNome/Downloads/obs_control_panel.html) e cole no OBS.

Clique em Aplicar. A doca vai aparecer, você pode arrastá-la e fixá-la na interface do OBS.

Opção B: Navegador Externo (Chrome/Edge)

Abra o arquivo obs_control_panel.html no seu navegador.

Importante: Para a integração com Holyrics funcionar via navegador externo, você pode precisar instalar uma extensão que permita CORS (Cross-Origin Resource Sharing) ou usar a opção A.

🔗 Conectando ao Holyrics

No Holyrics, certifique-se que o servidor de visualização está ativo.

No Painel de Controle (seção Bíblia), insira o endereço local do Holyrics.

Geralmente é algo como: http://192.168.x.x:2020/view/text3

Clique em "SINCRONIZAR COM HOLYRICS".

Se a "bolinha" ficar verde, está conectado! Ao mudar um versículo no Holyrics, ele aparecerá no OBS.
